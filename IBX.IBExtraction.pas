...
type
  ...
  TIBExtraction = class(TComponent)
  private
    FBase : TIBBase;
    FMetaData: TStrings;
...
function TIBExtraction.GetArrayField(FieldName: String): String;
var
  qryArray : TIBSQL;
begin
...
procedure TIBExtraction.checkAssigned;
begin
...
constructor TIBExtraction.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TIBExtraction.Destroy;
begin
  FMetaData.Free;
  FDatabasEInfo.Free;
...
function TIBExtraction.ExtractDDL(Flag: Boolean; TableName: String) : Boolean;
var
	DidConnect : Boolean;
	DidStart : Boolean;
begin
...
function TIBExtraction.ExtractListTable(RelationName, NewName: String; DomainFlag: Boolean; aQry: TIBSQL = nil): Boolean;
const
...
begin
  ...
  begin
    ...
    if NewName <> '' then { do not localize }
      FMetaData.Add(Format(TableBase, [lowercase(NewName)])) { do not localize }
    else
      FMetaData.Add(Format(TableBase, [lowercase(RelationName)])); { do not localize }
  ...
  begin
    Column := '  ' + Lowercase(qryMain.FieldByName('RDB$FIELD_NAME').AsTrimString) + TAB; { do not localize }
    ...
    begin
      ...
      begin
          ...
          if (FieldType = blr_short) and (FieldScale < 0) then
            ...
            if (FieldType = blr_long) and (FieldScale < 0) then
              ...
              if (FieldType = blr_double) and (FieldScale < 0) then
                ...
                Column := Column + LowerCase(ColumnTypes.Items[qryMain.FieldByName('RDB$FIELD_TYPE').AsShort]);
        end;
        ...
        { International character sets }
        if ((FieldType in [blr_text, blr_varying])
        or (FieldType = blr_blob))
        and (not qryMain.FieldByName('RDB$CHARACTER_SET_ID').IsNull) and
        { do not localize }
...
procedure TIBExtraction.ExtractListView(ViewName: String);
const
...
function TIBExtraction.GetCharacterSets(CharSetId, Collation: Short;
...
function TIBExtraction.GetDatabase: TIBDatabase;
begin
  result := FBase.Database;
end;

 function TIBExtraction.GetDialect: Integer;
begin
...
function TIBExtraction.GetIndexSegments(IndexName: String): String;
const
  IndexNamesSQL =
...
function TIBExtraction.GetTransaction: TIBTransaction;
begin
...
procedure TIBExtraction.ListGrants;
const
...
procedure TIBExtraction.ListProcs(ProcedureName : String; AlterOnly : Boolean);
const
...
procedure TIBExtraction.ListAllTables(flag: Boolean);
const
  TableSQL =
       'SELECT * ' +
...
procedure TIBExtraction.ListTriggers(ObjectName : String; ExtractType : TExtractType);
const
...
procedure TIBExtraction.ListCheck(ObjectName : String; ExtractType : TExtractType);
const
...
procedure TIBExtraction.ListCreateDb(TargetDb : String);
const
  CharInfoSQL =
...
procedure TIBExtraction.ListDomains(ObjectName: String; ExtractType : TExtractType);
const
  DomainSQL =
...
procedure TIBExtraction.ListException(ExceptionName : String = '');
const
  ExceptionSQL =
...
procedure TIBExtraction.ListFilters(FilterName : String = '');
const
  FiltersSQL =
...
procedure TIBExtraction.ListForeign(ObjectName : String; ExtractType : TExtractType);
const
...
procedure TIBExtraction.ListFunctions(FunctionName : String = '');
const
  FunctionSQL =
...
procedure TIBExtraction.ListGenerators(GeneratorName : String = '');   {do not localize}
const
  GeneratorSQL =
...
procedure TIBExtraction.ListGrants(ObjectName: string);
begin
  if ObjectName = '' then
    ListGrants
  else
...
procedure TIBExtraction.ListIndex(ObjectName : String; ExtractType : TExtractType);
const
  IndexSQL =
...
procedure TIBExtraction.ListViews(ViewName : String);
const
  ViewSQL =
...
function TIBExtraction.PrintSet(var Used: Boolean) : String;
begin
  if not Used then
  begin
...
function TIBExtraction.PrintValidation(ToValidate: String;
  flag: Boolean): String;
var
  IsSQL : Boolean;
begin
...
procedure TIBExtraction.SetDatabase(const Value: TIBDatabase);
begin
  FBase.Database := Value;
  FDatabaseInfo.Database := Value;
end;

procedure TIBExtraction.SetOverrideSQLDialect(const Value: Integer);
begin
...
procedure TIBExtraction.SetTransaction(const Value: TIBTransaction);
begin
...
procedure TIBExtraction.ExtractObject(ObjectType : TExtractObjectTypes;
...
function TIBExtraction.GetFieldType(FieldType, FieldSubType, FieldScale,
...
procedure TIBExtraction.ShowGrants(MetaObject, Terminator: String);
const
...
procedure TIBExtraction.ShowGrantRoles(Terminator: String);
const
  RoleSQL =
...
procedure TIBExtraction.GetProcedureArgs(Proc: String; args : TStrings);
const
...
procedure TIBExtraction.BuildConnectString;
const
...
procedure TIBExtraction.ListData(ObjectName: String);
const
...
begin
  ...
  try
    qrySelect.SQL.Text := Format(SelectSQL, [QuoteIdentifier(GetDialect, ObjectName)]);
    qrySelect.ExecQuery;
    Fields := '';
    for i := 0 to qrySelect.Current.Count - 1 do
      ...
      begin
        ...
        Fields := Fields + Lowercase(FieldName);
      end;
    while not qrySelect.Eof do
    begin
      Line := 'INSERT INTO ' + lowercase(ObjectName) + ' (';  {do not localize}
      ...
      begin
        ...
        case qrySelect.Fields[i].SQLType of
          ...
          SQL_TYPE_DATE :
          begin
            DateTimeToString(DateStr, 'yyyy-mm-dd', qrySelect.Fields[i].AsDate, AFormatSettings); {do not localize}
          ...
          SQL_TIMESTAMP :
          begin
            DateTimeToString(DateStr, 'yyyy-mm-dd hh:mm:ssss', qrySelect.Fields[i].AsDateTime, AFormatSettings); {do not localize}
            Values := Values + QuotedStr(DateStr);
          end;
          SQL_SHORT,
          SQL_LONG,
          ...
          SQL_D_FLOAT:
          begin
            Values := Values + StringReplace(qrySelect.Fields[i].AsTrimString,',','.',[rfReplaceAll]);
          end;
          SQL_BOOLEAN: begin
            if qrySelect.Fields[i].AsTrimString.ToLower = 'f' then
              Values := Values + 'false'
            else
            if qrySelect.Fields[i].AsTrimString.ToLower = 't' then
              Values := Values + 'true'
            else
              Values := Values + 'null';
          end;
...
procedure TIBExtraction.ListRoles(ObjectName: String);
const
  RolesSQL =
...
procedure TIBExtraction.ListSubscriptions(SubscriptionName: String);
const
...
Procedure TIBExtraction.ListEUAUsers;
const
...
Procedure TIBExtraction.ListEncryptions(EncryptionName : String);
const
...
function TIBExtraction.GetFieldLength(sql: TIBSQL): Integer;
begin
...
function TIBExtraction.CreateIBSQL: TIBSQL;
begin
